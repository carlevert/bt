...
When the client has finished ``playing'' a round it always get a new highscore, ie it always wins. The new highscore is better than the old one by a number drawn from a uniform pseudorandom distribution $1-1000$.
T
o be able to measure the relative error of the estimates the client starts by asking for a list of all highscores. When the client sends a new highscore to the server it also updates its local highscore list. When the server responds with the rank estimate for the new score, that estimate is compared with the real rank and ultimately used for calculating the relative error.
...
to the Java-class (\texttt{@Entity} on the class and \texttt{@Id} on the String or Long field that will be used for creating the key) objects can be saved with a statement like \texttt{ofy().save().entity(theObject)}. \textbf{Jackson} is a library for parsing JSON-data as well as mapping JSON to Java objects.

\section{Limitations and ways of improving the experiment}

To be able to process the outcome of this experiment, several runs with different random seeds would make it possible to analyze the result with statistical methods.

The experiment is only run on a local development server. While there is no obvious reason to believe that the results and conclusions could be applied to a production environment in the cloud in principle, there may be factors that need to be considered.
